...
在使用板子，您可以使用 :ref:`I2C类<machine.I2C>` 函数与I2C总线上的设备进行交互。在大多数情况下，您将充当I2C“主设备”，可以与其他I2C设备读写数据。
...
``i2c.writeto(addr, buf)`` 为I2C写操作函数，向 I2C设备为 ``addr`` ，发送 ``buf`` 缓存字节。板子需要向SHT20发送 ``0xf3`` 字节，告诉它，我需要读取温度数据，延时70毫秒后
